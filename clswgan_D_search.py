...
parser.add_argument('--cls_weight', type=float, default=0.1, help='weight of the classification loss')
parser.add_argument('--lr', type=float, default=0.0005, help='learning rate to train GANs ')
...
parser.add_argument('--num_individual', type=int, default=40, help='# num of population')
...
parser.add_argument('--regular_weight', type=float, default=2.0, help='weight regular')

opt = parser.parse_args()
print(opt)

time_str = time.strftime("%m-%d-%H-%M", time.localtime())
exp_dir =  opt.outf + opt.dataset + '_' + time_str + '_D_search_{}'.format(opt.regular_weight)
...
with open(exp_dir + '/D_searching_init.txt', 'a') as file:
    file.write(str(opt))
...
def set_requires_grad(nets, g):
    for p in nets.network.parameters(): # set requires_grad to False
        p.requires_grad = False
    for i in range(g.shape[0]):
        j = np.argmax(g[i,:])
        for p in nets.network.layers[i].layers[j].parameters(): # set requires_grad to False
            p.requires_grad = True

def calc_gradient_penalty(netD, genotype, real_data, fake_data, input_att):
    ...
    disc_interpolates = netD(interpolates, Variable(input_att), genotype)
...
def sample_darts(index, batch = opt.batch_size):
    batch_feature, batch_label, batch_att = data.next_batch_darts(index, batch)
    input_res.copy_(batch_feature).to(opt.device)
    input_att.copy_(batch_att).to(opt.device)
    input_label.copy_(util.map_label(batch_label, data.seenclasses)).to(opt.device)
...
def search_evol_arch2(gen_net, dis_net, genotypes, gan_alg):
    ...
    noise_val = torch.FloatTensor(len(valid_index), opt.nz).to(opt.device)
    noise_val.normal_(0, 1)
    noise_valv = Variable(noise_val).to(opt.device)

    for idx, genotype_D in enumerate(tqdm(genotypes, desc='searching best genotype')):
        Wd_value, para_value  = validate(gen_net, dis_net, genotype_D, noise_valv)
        fit_values[idx] = Wd_value + opt.regular_weight*para_value
        Wd_values[idx] = Wd_value
        para_values[idx] = para_value
    keep = np.argsort(fit_values)[0:keep_N]
    gan_alg.update(genotypes[keep])
    print("best Wasserstein_dist:{}".format(fit_values[keep[0]]))
    return genotypes[keep]

def validate(gen_net, dis_net, genotype_D, noise_valv):
    input_resv_val = Variable(input_res_val).to(opt.device)
    input_attv_val = Variable(input_att_val).to(opt.device)
    fake = gen_net(noise_valv, input_attv_val)
...
#==================================train GAN===================================#
    for i in tqdm(range(0, data.ntrain * opt.num_train, opt.batch_size), desc='training'):
        ...
        if epoch<opt.warmup_nepoch:
            ...
            n=random.randint(0, genotypes.shape[0]-1)
            g = genotypes[n,:,:]
            set_requires_grad(netD_search, g)
        for iter_d in range(opt.critic_iter):
            ...
            criticD_real = netD_search(input_resv_train, input_attv_train, g)
            ...
            fake = netG(noisev, input_attv_train)
            criticD_fake = netD_search(fake.detach(), input_attv_train, g)
            criticD_fake = criticD_fake.mean()
            criticD_fake.backward(one)

            # gradient penalty
            gradient_penalty = calc_gradient_penalty(netD_search, g, input_res, fake.data, input_att)
        ...
        noisev = Variable(noise).to(opt.device)
        fake = netG(noisev, input_attv_train)
        criticG_fake = netD_search(fake, input_attv_train, g)
...
#==================================search genarator===================================#
    ...
    netD_search.eval()
    if epoch >= opt.warmup_nepoch:        
        genotypes = search_evol_arch2(netG, netD_search, genotypes, gan_alg)
        ...
        if opt.gzsl:
            ...
            print('unseen class accuracy= ', cls2.acc.item())
        # Zero-shot learning
        else:
            syn_feature, syn_label = generate_syn_feature(netG, data.unseenclasses, data.attribute, opt.syn_num)
        ...
        if best_acc < acc:
            ...
            best_acc = acc
            print('Best')
            print('discriminator', genotypes[0])
            # Plot the architecture picture
            if opt.outf:
                plot_genotype(opt,
                    'd',
                    genotypes[0],
                    file_name=opt.dataset+'_NEW_D_' + str(epoch),
                    figure_dir=exp_dir,
                    save_figure=True
                )
                print('Figure saved.') 
...
for i in range(opt.num_individual):
    ...
    if best_acc < acc:
        ...
        print('Best')
        print('discriminator', genotypes[i])
        # Plot the architecture picture
        if opt.outf:
            plot_genotype(opt,
                'd',
                genotypes[i],
                file_name=opt.dataset+'_NEW_D_' + 'final',
                figure_dir=exp_dir,
                save_figure=True
            )
            print('Figure saved.') 

np.save(opt.geo_dir+"/{}_D.npy".format(opt.dataset), best_genotype)
print(opt.geo_dir+"/{}_D.npy".format(opt.dataset))
